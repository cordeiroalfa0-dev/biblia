...
import { 
  ...
  Maximize2, Eye, Play, HelpCircle, Wand2, Hammer, ScrollText
} from 'lucide-react';
import { 
...
const SACRED_MOMENTS = [
  ...
  "O Sacrifício de Isaque", "A Escada de Jacó", "José e a Túnica Colorida", 
  "Moisés e a Sarça Ardente", "As Dez Pragas do Egito", "A Travessia do Mar Vermelho",
  "O Maná no Deserto", "Os Dez Mandamentos", "Os Muros de Jericó", "Sansão e as Colunas",
  "Davi contra o Gigante Golias", "O Templo de Salomão", "Elias no Carro de Fogo",
  "Eliseu e o Machado Flutuante", "A Restauração de Jó", "Salmo 23: O Bom Pastor",
  "A Visão de Isaías do Trono", "Daniel na Cova dos Leões", "Jonas e o Grande Peixe",
  "Neemias Reconstruindo Jerusalém", "Ester Salvando seu Povo", "A Estrela de Belém",
  "O Nascimento de Jesus", "O Batismo no Jordão", "A Tentação no Deserto",
  "As Bodas de Caná (Água em Vinho)", "O Sermão da Montanha", "Jesus Acalma a Tempestade",
  "Caminhando sobre as Águas", "A Multiplicação dos Pães", "A Transfiguração",
  "A Ressurreição de Lázaro", "A Entrada Triunfal em Jerusalém", "A Última Ceia",
  "O Getsêmani", "A Crucificação no Calvário", "A Ressurreição ao Terceiro Dia",
  "A Ascensão aos Céus", "Pentecostes: Línguas de Fogo", "A Visão de Pedro (O Lençol)",
  "Paulo na Estrada de Damasco", "O Naufrágio de Paulo em Malta", "Hino ao Amor (Coríntios)",
...
const VisionModal = ({ image, onClose }: { image: any, onClose: () => void }) => (
  <div className="fixed inset-0 z-[250] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-4 lg:p-20 animate-in zoom-in-95" onClick={onClose}>
    <div className="relative w-full max-w-3xl flex flex-col items-center gap-8" onClick={e => e.stopPropagation()}>
      <button onClick={onClose} className="absolute -top-12 right-0 p-3 text-white/50 hover:text-white"><X size={32} /></button>
      <div className="w-full aspect-square rounded-[3rem] overflow-hidden border border-white/10 shadow-[0_0_100px_rgba(245,158,11,0.1)] bg-black">
        <img src={image.image_data} alt={image.theme} className="w-full h-full object-contain" />
      </div>
      <div className="text-center space-y-2">
        <h4 className="text-3xl lg:text-5xl font-black bible-text italic text-white uppercase tracking-tighter">{image.theme}</h4>
        <p className="text-[10px] text-amber-500 font-black uppercase tracking-[0.4em]">Visão Sagrada • Nível {image.level}</p>
      </div>
    </div>
  </div>
);
...
const BatchGenerator = ({ type, onFinished }: { type: 'images' | 'crosswords', onFinished: () => void }) => {
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      if (type === 'images') {
        ...
        if (!exists) {
          ...
          setLog(prev => [`Pintando Nível ${i}: ${theme}`, ...prev.slice(0, 5)]);
    ...
    <div className="bg-white/[0.02] border border-white/5 rounded-[3rem] p-10 space-y-8 animate-in fade-in zoom-in-95">
      <div className="flex flex-col items-center text-center gap-4">
        <div className="w-20 h-20 bg-amber-600 rounded-full flex items-center justify-center text-black shadow-2xl">
          {status === "running" ? <Loader2 className="animate-spin" size={32} /> : type === 'images' ? <Wand2 size={32} /> : <ScrollText size={32} />}
        </div>
      ...
      {status === 'idle' && (
        <button onClick={startBatch} className="w-full py-6 bg-amber-600 hover:bg-amber-500 text-black font-black uppercase text-[10px] tracking-[0.5em] rounded-2xl transition-all shadow-xl">
          Iniciar Revelação
        </button>
      )}
      ...
      {status === 'running' && (
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            {log.map((line, idx) => (
              <p key={idx} className={`text-[10px] font-bold uppercase tracking-widest ${idx === 0 ? 'text-amber-500 animate-pulse' : 'text-gray-600'}`}>{line}</p>
            ))}
          </div>
        </div>
      )}
      ...
      {status === 'finished' && (
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-[10px] font-black text-white uppercase tracking-[0.5em]">Tudo foi Consumado!</span>
        </div>
      )}
    </div>
  );
...
const MosaicPuzzle = ({ level, onComplete, onExit }: { level: number, onComplete: () => void, onExit: () => void }) => {
  ...
  useEffect(() => {
    const initMosaic = async () => {
      ...
      if (stored) {
        ...
        const selectedMoment = SACRED_MOMENTS[(level - 1) % SACRED_MOMENTS.length];
  ...
  if (loading) return <LoadingOverlay message="Revelando a Obra do Criador..." />;
  if (!imageUrl) return <div className="text-center p-20 opacity-20">A visão não pôde ser revelada.</div>;
    ...
    <div className="fixed inset-0 bg-[#020202] z-[100] flex flex-col items-center p-4 lg:p-12 animate-in fade-in duration-700 overflow-y-auto">
      <div className="w-full max-w-6xl flex flex-col items-center gap-6 mt-4 mb-32">
        <div className="text-center space-y-1">
          <h3 className="text-2xl lg:text-5xl font-black bible-text italic text-white uppercase tracking-tighter">Mosaico Sagrado</h3>
          <p className="text-[9px] text-amber-500 font-black uppercase tracking-[0.4em]">{theme} • Nível {level}</p>
        </div>
            ...
            <div className="grid grid-cols-5 gap-0.5 bg-white/[0.03] p-1 rounded-2xl border border-white/10 shadow-2xl aspect-square relative">
              {placedPieces.map((pieceId, idx) => {
                ...
                return (
                  <div key={idx} onDragOver={(e) => e.preventDefault()} onDrop={(e) => onDrop(e, idx)}
                    ...
                    {pieceId ? (
                      <div className="w-full h-full" style={{ backgroundImage: `url(${imageUrl})`, backgroundSize: '500% 500%', backgroundPosition: `${(idx%gridSize/(gridSize-1))*100}% ${(Math.floor(idx/gridSize)/(gridSize-1))*100}%` }} />
              ...
              {showHint && (
                <div className="absolute inset-0 z-10 p-1 animate-in fade-in duration-300">
                  <img src={imageUrl} className="w-full h-full object-cover rounded-xl opacity-80" />
                </div>
              )}
            </div>
          ...
          <div className="w-full max-w-[340px] lg:max-w-sm bg-white/[0.02] backdrop-blur-3xl p-6 rounded-[2rem] border border-white/5 space-y-4">
             ...
             <div className="grid grid-cols-5 gap-2 max-h-[160px] lg:max-h-none overflow-y-auto p-1">
                {availablePieces.map((pId) => {
                  ...
                  return (
                    <div key={pId} draggable onDragStart={(e) => e.dataTransfer.setData("pieceId", pId.toString())}
                      className="aspect-square rounded-md border border-white/10 cursor-grab active:scale-90 transition-all overflow-hidden"
                      style={{ backgroundImage: `url(${imageUrl})`, backgroundSize: '500% 500%', backgroundPosition: `${(c/(gridSize-1))*100}% ${(r/(gridSize-1))*100}%` }}
                    />
                  );
                })}
             </div>
      ...
      <div className="fixed bottom-6 left-0 right-0 flex justify-center z-[110] px-6">
        <button onClick={onExit} className="flex items-center gap-3 bg-black/90 hover:bg-black px-8 py-4 rounded-full border border-white/10 shadow-2xl transition-all">
          <LogOut size={18} className="text-amber-500" />
          <span className="text-[10px] font-black text-white uppercase tracking-widest">Sair</span>
        </button>
      </div>
    </div>
  );
...
const CrosswordGame = ({ level, onComplete }: { level: number, onComplete: () => void }) => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in max-w-4xl mx-auto px-4 pb-20">
      <div className="text-center space-y-1">
        <h3 className="text-3xl font-black bible-text italic text-white uppercase tracking-tighter">Cruzadinha</h3>
        <p className="text-[9px] text-amber-500 font-black uppercase tracking-widest">{puzzle.theme}</p>
      </div>
      <div className="flex flex-col lg:flex-row gap-8 items-start justify-center">
        <div className="grid grid-cols-12 gap-0.5 bg-white/5 p-2 rounded-xl border border-white/10">
          {userGrid.map((row, r) => row.map((cell, c) => {
            ...
            if (!isWordCell) return <div key={`${r}-${c}`} className="w-5 h-5 lg:w-8 lg:h-8 bg-black/40 rounded-sm" />;
            return (
                ...
                onChange={(e) => {
                  ...
                  puzzle.words.forEach((w: any) => {
                    for (let i = 0; i < w.word.length; i++) {
                      ...
                      if (newGrid[row][col] !== w.word[i].toUpperCase()) win = false;
                    }
                  });
                  if (win) onComplete();
                }}
        ...
        <div className="flex-1 space-y-4 max-w-sm">
          {['across', 'down'].map(dir => (
            <div key={dir} className="space-y-2">
              <h4 className="text-[8px] font-black text-amber-500 uppercase tracking-widest border-l border-amber-500 pl-3">{dir === 'across' ? 'Horizontal' : 'Vertical'}</h4>
              {puzzle.words.filter((w: any) => w.direction === dir).map((w: any, i: number) => (
                <p key={i} className="text-[10px] text-gray-500 leading-relaxed bg-white/[0.02] p-3 rounded-lg border border-white/5">{w.clue}</p>
              ))}
            </div>
          ))}
        </div>
...
const App: React.FC = () => {
  ...
  const [gamesSubTab, setGamesSubTab] = useState<'mosaic' | 'crossword' | 'generator'>('mosaic');
  ...
  useEffect(() => {
    testDatabaseConnection().then(res => { if (res.success) loadContent(); });
  ...
  const loadContent = async () => {
    ...
    if (activeTab === 'bible') {
      ...
      if (data.length === 0) {
        ...
        if (cData.length) {
          await saveVerses(cData.map(v => ({ book_name: currentBook, chapter: currentChapter, verse: v.verse, text: v.text })));
  ...
  return (
    <div className="flex h-screen w-full bg-[#020202] text-[#e5e5e5] font-sans overflow-hidden">
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
               <div className="bg-black/40">
                  <button onClick={() => setActiveTab('bible')} className={`w-full flex items-center gap-5 p-6 border-b border-white/[0.02] ${activeTab === 'bible' ? 'bg-amber-600 text-black' : 'text-gray-500'}`}>
                    <BookOpen size={24} /><span className="text-[11px] font-black uppercase tracking-widest">Escrituras</span>
                  </button>
                  <button onClick={() => setActiveTab('games')} className={`w-full flex items-center gap-5 p-6 border-b border-white/[0.02] ${activeTab === 'games' ? 'bg-amber-600 text-black' : 'text-gray-500'}`}>
                    <Gamepad2 size={24} /><span className="text-[11px] font-black uppercase tracking-widest">Jornada</span>
                  </button>
                  <button onClick={() => setActiveTab('harpa')} className={`w-full flex items-center gap-5 p-6 border-b border-white/[0.02] ${activeTab === 'harpa' ? 'bg-amber-600 text-black' : 'text-gray-500'}`}>
                  ...
                  {activeTab === 'bible' && (
                    <div className="p-4 space-y-4">
                      ...
                      {filteredBooks.map(book => (
                        <button key={book.name} onClick={() => { setCurrentBook(book.name); setCurrentChapter(1); setIsSidebarOpen(false); }} className={`w-full text-left py-4 px-6 rounded-2xl flex items-center justify-between ${currentBook === book.name ? 'bg-white/[0.03] text-white' : 'text-gray-500'}`}>
                    ...
                    <div className="p-8 space-y-8">
                      <div className="flex flex-col gap-2">
                        <div className="flex bg-white/5 p-1 rounded-2xl">
                          <button onClick={() => setGamesSubTab('mosaic')} className={`flex-1 py-4 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${gamesSubTab === 'mosaic' ? 'bg-amber-600 text-black' : 'text-gray-500'}`}>Mosaicos</button>
                          <button onClick={() => setGamesSubTab('crossword')} className={`flex-1 py-4 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${gamesSubTab === 'crossword' ? 'bg-amber-600 text-black' : 'text-gray-500'}`}>Cruzadas</button>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => { setGamesSubTab('generator'); setGenType('images'); }} className={`flex-1 py-3 rounded-xl text-[8px] font-black uppercase tracking-widest border border-amber-500/20 text-amber-500 flex items-center justify-center gap-2 ${gamesSubTab === 'generator' && genType === 'images' ? 'bg-amber-500/10' : ''}`}>
                             <ImageIcon size={12} /> Visões
                          </button>
                          <button onClick={() => { setGamesSubTab('generator'); setGenType('crosswords'); }} className={`flex-1 py-3 rounded-xl text-[8px] font-black uppercase tracking-widest border border-amber-500/20 text-amber-500 flex items-center justify-center gap-2 ${gamesSubTab === 'generator' && genType === 'crosswords' ? 'bg-amber-500/10' : ''}`}>
                          ...
                          {Array.from({ length: 60 }, (_, i) => i + 1).map(lv => {
                            ...
                            return (
                              <div key={lv} className="relative group">
                                <button onClick={() => { if (gamesSubTab === 'mosaic') { setCurrentMosaicLevel(lv); setIsMosaicActive(true); } else setCurrentGameLevel(lv); setIsSidebarOpen(false); }}
                                ...
                                {gamesSubTab === 'mosaic' && vision && (
                                  <button onClick={(e) => { e.stopPropagation(); setSelectedVision(vision); }} className="absolute -top-1 -right-1 bg-amber-500 text-black p-1 rounded-full shadow-lg hover:scale-125 transition-all">
                                    <Eye size={10} />
                                  </button>
                                )}
                  ...
                  {activeTab === 'harpa' && (
                    <div className="p-4 space-y-4">
                      <div className="sticky top-0 bg-[#080808] pb-4 z-10 px-2">
                        <input type="text" placeholder="Buscar hino..." value={hymnSearch} onChange={e => setHymnSearch(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-6 text-xs font-bold outline-none text-white focus:border-amber-500/30" />
                      </div>
                      {hymns.map(h => (
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
          ...
          <header className="h-24 lg:h-32 glass border-b border-white/5 flex items-center justify-between px-6 lg:px-16 z-40">
            <div className="flex items-center gap-6">
                ...
                <h2 className="text-2xl lg:text-4xl font-black bible-text italic text-white uppercase tracking-tighter">
                  {activeTab === 'bible' ? <><span className="text-amber-500">{currentBook}</span> <span className="text-gray-800 font-light">/</span> {currentChapter}</> : activeTab === 'harpa' ? 'Harpa' : 'Jornada'}
                </h2>
            </div>
            <div className="flex items-center gap-3">
                <div className="flex bg-white/5 p-1 rounded-xl">
                  <button onClick={() => setFontScale(s => Math.max(0.6, s-0.1))} className="p-2 text-gray-500 hover:text-white"><ZoomOut size={18} /></button>
                  <button onClick={() => setFontScale(s => Math.min(2.5, s+0.1))} className="p-2 text-gray-500 hover:text-white"><ZoomIn size={18} /></button>
                </div>
            </div>
          </header>
        )}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
           {loading ? <LoadingOverlay message="Conectando aos Registros Sagrados..." /> : (
             <div className={`${isMosaicActive ? '' : 'max-w-4xl mx-auto py-12 lg:py-20 px-6 pb-40'}`}>
                {activeTab === 'bible' ? (
                  ...
                  selectedHymn ? (
                    <div className="space-y-16 text-center animate-in slide-in-from-bottom-12">
                       ...
                       <div className="bg-[#050505] rounded-[3rem] p-12 lg:p-20 border border-white/5 text-gray-300 italic text-2xl lg:text-3xl leading-[2.2] whitespace-pre-line" style={{ fontSize: `${24 * fontScale}px` }}>
                          {selectedHymn.lyrics}
                       </div>
                    </div>
                  ...
                  : gamesSubTab === 'generator' ? ( <div className="max-w-md mx-auto"><BatchGenerator type={genType} onFinished={loadContent} /></div> )
          ...
          <div className="fixed inset-0 bg-black/95 z-[200] flex items-center justify-center p-8 backdrop-blur-3xl animate-in zoom-in-95">
             <div className="w-full max-w-sm bg-[#0a0a0a] rounded-[4rem] border border-amber-500/30 p-16 text-center space-y-10">
                ...
                <div className="space-y-2">
                   ...
                   <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest">Nível Concluído</p>
                </div>
        ...
        {selectedVision && <VisionModal image={selectedVision} onClose={() => setSelectedVision(null)} />}
      </main>
